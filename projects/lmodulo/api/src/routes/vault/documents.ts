...
export default async function documentsRoutes(app: FastifyInstance) {
  ...
  app.get<{
    ...
    const filter: Record<string, unknown> = { ...visibilityFilter(role) };
  ...
  app.post('/', {
    ...
    const storageKey = `vault/${docId.toString()}`;
  ...
  app.get<{ Params: { id: string } }>('/:id', {
    ...
    const filter: Record<string, unknown> = { _id, ...visibilityFilter(role) };
  ...
  app.patch<{
    ...
    const result = await db.collection(COL_DOCS).updateOne({ _id }, { $set });
  ...
  app.delete<{ Params: { id: string } }>('/:id', {
    ...
    const doc = await db.collection(COL_DOCS).findOne({ _id });
    if (!doc) return reply.notFound('Document not found');
  ...
  app.post<{ Params: { id: string } }>('/:id/versions', {
    ...
    const verId = new ObjectId();
    const storageKey = `vault/${_id.toString()}`;
    const url = await storage.save(fileData.filename, fileData.buffer, fileData.mimetype, storageKey);
