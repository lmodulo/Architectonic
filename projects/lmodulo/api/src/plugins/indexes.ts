...
export default async function ensureIndexes(app: FastifyInstance) {
  app.addHook('onReady', async () => {
    ...
    // time_entries
    await db.collection('time_entries').createIndex({ userId: 1, date: 1 });
    await db.collection('time_entries').createIndex({ userId: 1, timerRunning: 1 });
    await db.collection('time_entries').createIndex({ taskId: 1, date: 1 });
    await db.collection('time_entries').createIndex({ sprintId: 1, date: 1 });
    await db.collection('time_entries').createIndex({ milestoneId: 1, date: 1 });
    ...
    await db.collection('finance_retainer_periods').createIndex({ subscriptionId: 1, status: 1 });
    await db.collection('finance_retainer_periods').createIndex({ subscriptionId: 1, periodStart: -1 });
    await db.collection('finance_retainer_periods').createIndex({ companyId: 1, status: 1 });
    ...
    await db.collection('finance_estimates').createIndex({ title: 'text' });

    // finance_expenses
    await db.collection('finance_expenses').createIndex({ expenseNumber: 1 }, { unique: true });
    await db.collection('finance_expenses').createIndex({ status: 1, expenseDate: -1 });
    await db.collection('finance_expenses').createIndex({ category: 1, expenseDate: -1 });
    ...
    await db.collection('contracts').createIndex({ title: 'text' });
