<script lang="ts">
  ...
  let editForm    = $state({
    ...
    blockedReason: '', dueDate: '',
    estimateHours: 0, // read-only reference
  });
              ...
              <tr class="border-b border-surface-200-800 last:border-0 odd:bg-transparent even:bg-black/[.025] dark:even:bg-white/[.035] hover:bg-black/[.05] dark:hover:bg-white/[.06] transition-colors">
                ...
                {#if hasPermission(data.user, 'agile_tasks', 'update')}
                  <td class="px-2 py-2.5 text-center">
                    <button
                      type="button"
                      class="btn-icon btn-sm hover:preset-tonal opacity-40 hover:opacity-100"
  ...
  <div transition:fade={{ duration: 200 }}
    ...
    <div transition:scale={{ duration: 300, start: 0.95, easing: cubicOut }}
      ...
      <header class="flex items-center justify-between px-6 pt-5 pb-3 border-b border-surface-200-800 shrink-0">
        ...
        <button type="button" class="btn-icon hover:preset-tonal" onclick={() => (editModal = false)}><X class="size-5"/></button>
      </header>
      <div class="p-6 space-y-4 overflow-y-auto flex-1">
        ...
        <div class="rounded-base bg-surface-200-800/40 px-3 py-2 text-xs opacity-60 flex items-center gap-2">
        ...
        <div class="space-y-1">
          <label class="label text-xs font-medium opacity-60 uppercase tracking-wide" for="et-title">Title *</label>
          <input id="et-title" type="text" class="input w-full" placeholder="Task title" bind:value={editForm.title} />
        </div>
        <div class="space-y-1">
          <label class="label text-xs font-medium opacity-60 uppercase tracking-wide" for="et-desc">Description</label>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="space-y-1">
            <label class="label text-xs font-medium opacity-60 uppercase tracking-wide" for="et-due">Due Date</label>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-1">
            <label class="label text-xs font-medium opacity-60 uppercase tracking-wide" for="et-act">Logged Hours</label>
            <input id="et-act" type="number" min="0" step="0.5" class="input w-full" bind:value={editForm.actualHours} />
          </div>
          <div class="space-y-1">
            <label class="label text-xs font-medium opacity-60 uppercase tracking-wide" for="et-rem">Remaining Hours</label>
        ...
        {#if editForm.status === 'Blocked'}
          <div class="space-y-1">
            <label class="label text-xs font-medium opacity-60 uppercase tracking-wide" for="et-reason">Blocked Reason *</label>
      ...
      <footer class="flex justify-end gap-3 px-6 pb-5 border-t border-surface-200-800 pt-3 shrink-0">
        <button type="button" class="btn preset-tonal" onclick={() => (editModal = false)}>Cancel</button>
        <button type="button" class="btn preset-filled-primary-500" disabled={savingEdit} onclick={saveEdit}>
