...
<div class="space-y-10">
  ...
  <div class="space-y-4">
    ...
    <pre class="bg-base-300 rounded-box p-4 text-sm overflow-x-auto leading-relaxed"><code>finance_estimates     ← client-approved quote; converts to an invoice on acceptance
  └── lineItems[]     ← embedded array of line items
  └── invoiceId?      ← set once converted to a finance_invoice

finance_invoices      ← one-off, recurring template, or auto-generated invoice
  └── lineItems[]     ← embedded array of line items
  └── estimateId?     ← set on invoices converted from an estimate
...
finance_expenses      ← internal business cost (hosting, software, contractors, etc.)
  ...
  <!-- Estimate Lifecycle -->
  <div class="space-y-4">
    <h2 class="text-xl font-semibold">Estimate Lifecycle</h2>
    ...
    <p class="text-sm opacity-70 leading-relaxed">
      Estimate numbers are auto-assigned on create in <code class="bg-base-300 px-1 rounded text-xs">EST-0001</code>, <code class="bg-base-300 px-1 rounded text-xs">EST-0002</code>, … format. Once an estimate has been converted, <code class="bg-base-300 px-1 rounded text-xs">invoiceId</code> is set and the "Convert to Invoice" button disappears — re-conversion is blocked at the API level (409 Conflict).
    </p>

    <h3 class="text-base font-semibold mt-2">finance_estimates</h3>
    <div class="overflow-x-auto">
      <table class="table table-sm w-full">
        <thead><tr class="bg-base-200"><th>Field</th><th>Type</th><th>Description</th></tr></thead>
        <tbody>
          <tr><td class="font-mono text-xs">estimateNumber</td><td class="text-xs opacity-60">string (auto)</td><td class="text-sm opacity-70"><code class="bg-base-300 px-1 rounded text-xs">EST-0001</code> format, assigned on create</td></tr>
          <tr><td class="font-mono text-xs">title</td><td class="text-xs opacity-60">string</td><td class="text-sm opacity-70">Optional descriptive name, e.g. "Website Redesign 2026"</td></tr>
          <tr><td class="font-mono text-xs">customerId</td><td class="text-xs opacity-60">ObjectId ref</td><td class="text-sm opacity-70">The client (user with role <code class="bg-base-300 px-1 rounded text-xs">customer</code>)</td></tr>
          <tr><td class="font-mono text-xs">companyId</td><td class="text-xs opacity-60">ObjectId ref?</td><td class="text-sm opacity-70">The company being quoted (from <code class="bg-base-300 px-1 rounded text-xs">crm_companies</code>); copied to the invoice on conversion</td></tr>
          <tr><td class="font-mono text-xs">lineItems</td><td class="text-xs opacity-60">&#123;description, quantity, unitPrice, amount&#125;[]</td><td class="text-sm opacity-70">Embedded; <code class="bg-base-300 px-1 rounded text-xs">amount = quantity × unitPrice</code></td></tr>
          <tr><td class="font-mono text-xs">subtotal / taxRate / taxAmount / total</td><td class="text-xs opacity-60">numbers</td><td class="text-sm opacity-70">Computed server-side on every write; copied to the invoice on conversion</td></tr>
          ...
          <tr><td class="font-mono text-xs">status</td><td class="text-xs opacity-60">draft | sent | accepted | declined | expired</td><td class="text-sm opacity-70">Estimate lifecycle state</td></tr>
          <tr><td class="font-mono text-xs">validUntil</td><td class="text-xs opacity-60">date?</td><td class="text-sm opacity-70">Expiry date for the quote</td></tr>
          <tr><td class="font-mono text-xs">notes</td><td class="text-xs opacity-60">string</td><td class="text-sm opacity-70">Optional memo; copied to the invoice on conversion</td></tr>
          <tr><td class="font-mono text-xs">invoiceId</td><td class="text-xs opacity-60">ObjectId ref?</td><td class="text-sm opacity-70">Set once converted — links back to the created invoice</td></tr>
          <tr><td class="font-mono text-xs">createdBy</td><td class="text-xs opacity-60">userId</td><td class="text-sm opacity-70">Staff member who created the estimate</td></tr>
        </tbody>
      </table>
    </div>
  </div>
    ...
    <div class="overflow-x-auto">
      <table class="table table-sm w-full">
        ...
        <tbody>
          ...
          <tr><td class="font-mono text-xs">estimateId</td><td class="text-xs opacity-60">ObjectId ref?</td><td class="text-sm opacity-70">Set when this invoice was converted from a <code class="bg-base-300 px-1 rounded text-xs">finance_estimates</code> document</td></tr>
  ...
  <div class="space-y-4">
    ...
    <div class="space-y-3">
      ...
      <div class="card bg-base-200 border border-base-300 rounded-box p-4 space-y-1">
        <p class="text-sm font-semibold">All Estimates <code class="bg-base-300 px-1 rounded text-xs">/folio/estimates</code></p>
        <p class="text-sm opacity-60 leading-relaxed">Paginated, sortable table of every estimate. Status filter tabs (All / Draft / Sent / Accepted / Declined / Expired). Accepted rows show an <strong>Invoice →</strong> link when <code class="bg-base-300 px-1 rounded text-xs">invoiceId</code> is set. Requires <code class="bg-base-300 px-1 rounded text-xs">finance_estimates:read</code>.</p>
      </div>
      <div class="card bg-base-200 border border-base-300 rounded-box p-4 space-y-1">
        <p class="text-sm font-semibold">Create Estimate <code class="bg-base-300 px-1 rounded text-xs">/folio/estimates/new</code></p>
        <p class="text-sm opacity-60 leading-relaxed">Optional title, customer selector, dynamic line items builder, tax rate, currency, valid-until date, status (Draft / Send to client), and notes. No recurrence — estimates are one-off quotes. Requires <code class="bg-base-300 px-1 rounded text-xs">finance_estimates:create</code>.</p>
      </div>
      <div class="card bg-base-200 border border-base-300 rounded-box p-4 space-y-1">
        <p class="text-sm font-semibold">Estimate Detail <code class="bg-base-300 px-1 rounded text-xs">/folio/estimates/[id]</code></p>
        <p class="text-sm opacity-60 leading-relaxed">Full view with line items, tax breakdown, and totals. Edit mode exposes title, status, valid-until date, and notes. Action buttons: <strong>Send to client</strong> (draft only), <strong>Accept / Decline</strong> (sent only), <strong>Convert to Invoice</strong> (any non-declined, non-expired estimate that hasn't already been converted), <strong>Delete</strong> (unless already converted). On conversion, the browser navigates directly to the new invoice.</p>
      </div>
      <div class="card bg-base-200 border border-base-300 rounded-box p-4 space-y-1">
  ...
  <div class="space-y-4">
    ...
    <div class="overflow-x-auto">
      <table class="table table-sm w-full">
        ...
        <tbody>
          <tr><td class="font-mono text-xs">GET</td><td class="font-mono text-xs">/finance/estimates</td><td class="text-xs opacity-60">requireAuth</td><td class="text-sm opacity-70">List estimates; customers auto-scoped. Query: <code class="bg-base-300 px-1 rounded text-xs">status</code>, <code class="bg-base-300 px-1 rounded text-xs">limit</code>, <code class="bg-base-300 px-1 rounded text-xs">skip</code>, <code class="bg-base-300 px-1 rounded text-xs">sort</code>, <code class="bg-base-300 px-1 rounded text-xs">sortDir</code></td></tr>
          <tr><td class="font-mono text-xs">POST</td><td class="font-mono text-xs">/finance/estimates</td><td class="text-xs opacity-60">finance_estimates:create</td><td class="text-sm opacity-70">Create estimate; returns 201 + the new document</td></tr>
          <tr><td class="font-mono text-xs">GET</td><td class="font-mono text-xs">/finance/estimates/:id</td><td class="text-xs opacity-60">requireAuth</td><td class="text-sm opacity-70">Single estimate; scope-checked for customers</td></tr>
