...
<div class="space-y-1">
  <svg viewBox="0 0 {width} {height}" width="100%" {height} class="block rounded overflow-hidden" aria-hidden="true">
    ...
    <!-- actual (done) -->
    {#if actPct > 0}
      <rect x="0" y="0" width={actPct} height={height} fill="var(--color-success-500)" rx="3"/>
    {/if}
    <!-- remaining -->
    {#if remPct > 0}
      <rect x={actPct} y="0" width={remPct} height={height} fill="var(--color-primary-500)" fill-opacity="0.5" rx="3"/>
    {/if}
  </svg>
  <div class="flex items-center gap-3 text-[10px] opacity-60">
    <span class="flex items-center gap-1">
      <span class="size-2 rounded-full bg-success-500 inline-block"></span>
      {fmtEffort(actual)} logged
    </span>
    <span class="flex items-center gap-1">
      <span class="size-2 rounded-full bg-primary-500/50 inline-block"></span>
