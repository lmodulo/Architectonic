<script lang="ts">
  ...
  $effect(() => {
    ...
    for (const entry of navItems) {
      if (isNavGroup(entry) && entry.children.some(c => path.startsWith(c.href))) {
  ...
  <div data-theme={APP_THEME} style={fontVars} class="h-screen flex overflow-hidden">
    ...
    >
      ...
      <div class="relative flex items-center gap-2 px-4 h-16 shrink-0">
        ...
        >
          <span
            style="font-family: var(--display); background: radial-gradient(circle, color-mix(in oklch, var(--color-primary) 20%, transparent) 0%, transparent 70%);"
            class="text-3xl font-bold text-base-content select-none w-10 h-10 flex items-center justify-center rounded-full"
          >A</span>
        </a>
        <button
          type="button"
      ...
      {#if data.user?.role !== 'customer'}
        <div class="border-b border-base-300 px-2 py-2 transition-opacity duration-150 overflow-hidden {sidebarExpanded ? 'opacity-100' : 'lg:opacity-0 lg:pointer-events-none'}">
    ...
    <div class="relative flex flex-col flex-1 min-w-0 overflow-hidden">
      ...
      <header class="flex items-center gap-3 px-4 h-16 shrink-0 bg-base-200 rounded-br-xl">
        ...
        <!-- Logo slot: reveals when sidebar is collapsed on desktop -->
        <div class="hidden lg:grid h-16 items-center transition-[grid-template-columns] duration-200 ease-in-out {sidebarExpanded ? 'grid-cols-[0fr]' : 'grid-cols-[1fr]'}">
          <div class="overflow-hidden h-full flex items-center">
            <div class="flex items-center gap-3 pr-0.5">
              <a href="/dashboard" class="flex items-center no-underline text-inherit">
                <Logo brandName={data.brandName ?? ''} brandLogo={data.brandLogo ?? ''} />
              </a>
            </div>
          </div>
        ...
        <div class="flex items-center gap-1">
          <div class="tooltip tooltip-bottom" data-tip={m.nav_messages()}>
          ...
          <div class="tooltip tooltip-bottom" data-tip={m.nav_notifications()}>
        ...
        <!-- Right divider: hides when sidebar is expanded -->
      ...
      {#if showOverlay}
        ...
        >
          <div class="card bg-base-200 shadow-xl px-5 py-3 flex items-center gap-3 pointer-events-auto">
            ...
            <span class="text-sm font-medium">{m.common_loading()}</span>
          </div>
        </div>
      {/if}
