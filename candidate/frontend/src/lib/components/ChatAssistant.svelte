  ...
  >
    ...
    <div bind:this={messagesEl} class="flex-1 overflow-y-auto p-4 space-y-3">
      ...
      {#if getMessages().length === 0 && !isLoading()}
        <p class="text-center text-sm opacity-40 mt-10">{m.assistant_placeholder()}</p>
      {/if}
      ...
      {#if isLoading()}
        <div class="flex justify-start">
          <div class="bg-base-200 px-3 py-2 rounded-2xl rounded-bl-sm">
            <svg class="size-4 animate-spin text-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-label={m.assistant_thinking()}>
    ...
    <footer class="px-3 pb-3 pt-2 border-t border-base-200 shrink-0">
      <div class="join w-full">
        <input
          ...
          disabled={isLoading()}
          aria-label={m.assistant_message_label()}
        />
        <button
          type="button"
