...
describe('Messages', () => {
  ...
  beforeAll(async () => {
    ...
    ({ userId: aliceId } = await createUserAndWorkspace(getDb(), { username: 'alice', email: 'alice@t.com', password: 'pw123456', role: 'owner' }));
    ({ userId: bobId   } = await createUserAndWorkspace(getDb(), { username: 'bob',   email: 'bob@t.com',   password: 'pw123456', role: 'contributor' }));
  ...
  describe('GET /messages (inbox)', () => {
    ...
    it('returns empty inbox for user with no received messages', async () => {
      // Create a third user who has never received a message
      await createUserAndWorkspace(getDb(), { username: 'charlie', email: 'charlie@t.com', password: 'pw123456', role: 'viewer' });
  ...
  describe('GET /messages/:threadId', () => {
    ...
    it('non-participant gets 403', async () => {
      // Create new user not part of this thread
      await createUserAndWorkspace(getDb(), { username: 'dave', email: 'dave@t.com', password: 'pw123456', role: 'viewer' });
