<script lang="ts">
  import { Building2, ChevronDown, ChevronUp, ChevronsUpDown, DollarSign, FileSignature, Plus } from 'lucide-svelte';
...
<div class="flex flex-col gap-4">
  ...
  {#if data.contracts.length === 0}
    <div class="text-center py-16 text-base-content/50">
      <FileSignature size={32} class="size-8 opacity-20 mx-auto mb-2" />
            ...
            {#snippet sortTh(label: string, field: string, cls = '')}
              <th class={cls}>
                <button type="button" class="flex items-center gap-1 hover:opacity-80 transition-opacity" onclick={() => toggleSort(field)}>
                  {label}
                  {#if sortField === field}
                    {#if sortDir === 'asc'}<ChevronUp size={12} class="size-3 opacity-70" />{:else}<ChevronDown size={12} class="size-3 opacity-70" />{/if}
                  {:else}
              ...
              <td class="hidden sm:table-cell">
                {#if c.companyName}
                  <span class="flex items-center gap-1 text-sm">
                    <Building2 size={12} class="size-3 opacity-50" />
                    {c.companyName}
                  </span>
                {:else}
