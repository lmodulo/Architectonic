<script lang="ts">
  import { ArrowLeft, Ban, Building2, CalendarDays, Check, DollarSign, Edit3, Plus, Printer, RefreshCcw, Send, Trash2, X } from 'lucide-svelte';
...
<div class="flex flex-col gap-6">
  ...
  <div class="flex items-start justify-between gap-3 flex-wrap">
    <div class="flex items-start gap-3">
      <a href="/contracts" class="btn btn-ghost btn-square btn-sm mt-0.5"><ArrowLeft size={16} class="size-4" /></a>
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          {:else if c.companyName}
            <span transition:fade={{ duration: 150 }} class="flex items-center gap-1 text-sm text-base-content/60">
              <Building2 size={12} class="size-3" />
              {c.companyName}
            </span>
          {/if}
        </div>
  ...
  <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
    <div class="card bg-base-200 p-3">
      ...
      {:else}
        <p transition:fade={{ duration: 150 }} class="font-medium text-sm flex items-center gap-1">
          <DollarSign size={12} class="size-3 opacity-50" />
    ...
    <div class="card bg-base-200 p-3">
      ...
      {:else}
        <p transition:fade={{ duration: 150 }} class="font-medium text-sm flex items-center gap-1">
          <CalendarDays size={12} class="size-3 opacity-50" />
            ...
            <div class="flex items-center justify-between gap-2">
              ...
              <div class="flex items-center gap-2">
                {#if s.status === 'pending' && canSend && s.id}
                  ...
                  >
                    <RefreshCcw size={12} class="size-3 {resendingId === s.id ? 'animate-spin' : ''}" />
                  </button>
                {/if}
