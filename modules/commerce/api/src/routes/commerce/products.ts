...
const DISCOUNT_TYPES = ['percentage', 'fixed', 'quantity_tier'] as const;
...
export default async function productsRoutes(app: FastifyInstance) {
  ...
  app.patch<{ Params: { id: string; sku: string }; Body: { adjustment: number } }>('/:id/variants/:sku/stock', {
    ...
    // Build update: only apply if stock won't go negative
    ...
    if (result.matchedCount === 0) {
      // Determine whether product/SKU exists or stock was insufficient
