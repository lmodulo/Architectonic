<script lang="ts">
  import { Check, ChevronDown, ChevronUp, ChevronsUpDown, Milestone, Pencil, X } from 'lucide-svelte';
  ...
  import {
    ...
    type AgileMilestone,
  } from '$lib/utils/companyMilestones';
        ...
        <div class="card bg-base-200 border border-base-300 rounded-box overflow-hidden">
          {#if milestones.length === 0}
            <div class="p-8 text-center">
              <Milestone size={32} class="size-8 opacity-20 mx-auto mb-2" />
                    ...
                    <th class="text-xs font-medium opacity-60 uppercase tracking-wide {cls}">
                      <button type="button" class="flex items-center gap-1 hover:opacity-80 transition-opacity {cls.includes('text-right') ? 'ml-auto' : ''}" onclick={() => toggleMsSort(field)}>
                        ...
                        {#if msSortField === field}
                          {#if msSortDir === 'asc'}<ChevronUp size={12} class="size-3 opacity-70" />{:else}<ChevronDown size={12} class="size-3 opacity-70" />{/if}
                        {:else}
