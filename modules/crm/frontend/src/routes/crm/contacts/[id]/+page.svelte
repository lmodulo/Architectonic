<script lang="ts">
  import { Check, Pencil, UserCheck, UserPlus, X } from 'lucide-svelte';
  ...
  async function saveEdit() {
    if (!editPhoneValid) { saveError = 'Enter a valid phone number'; return; }
    ...
    try {
      ...
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
        saveError = (d as any).message === 'phone_invalid' ? 'Enter a valid phone number' : ((d as any).message ?? 'Save failed');
        return;
      }
  ...
  <div class="flex items-center justify-between gap-4">
    ...
    <div class="flex gap-2 flex-wrap">
      {#if hasPermission(data.user, 'crm_contacts', 'update') && !editing}
        <button class="btn btn-ghost btn-sm" onclick={startEdit}><Pencil size={16} class="size-4" /> Edit</button>
      {/if}
      {#if contact.isUser}
        <span class="badge badge-primary gap-1.5 px-3 py-2.5">
          <UserCheck size={14} class="size-3.5" /> Client Portal Active
        </span>
