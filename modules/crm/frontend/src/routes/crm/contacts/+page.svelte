<script lang="ts">
  ...
  async function save() {
    ...
    if (!formPhoneValid) { saveError = 'Enter a valid phone number'; return; }
...
<div class="space-y-4">
  <div class="flex items-start justify-between gap-4">
    ...
    <div class="flex flex-col items-end gap-2">
      <div class="relative">
        <Search size={14} class="size-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 opacity-40" />
        <input
      ...
      {#if hasPermission(data.user, 'crm_contacts', 'create')}
        <button class="btn btn-primary btn-sm" onclick={openModal}>
          <Plus size={16} class="size-4" /> New Contact
        </button>
      {/if}
    </div>
  </div>
  ...
  {#if contacts.length === 0}
    <div class="card bg-base-200 border border-base-300 rounded-box p-10 text-center opacity-50">
      <Users size={32} class="size-8 opacity-40 mx-auto mb-2" />
