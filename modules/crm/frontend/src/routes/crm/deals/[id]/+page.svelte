<script lang="ts">
  import { Check, FileText, Pencil, Plus, Trash2, X } from 'lucide-svelte';
    ...
    <div class="flex items-center gap-2 shrink-0">
      {#if canPropose && customers.length > 0}
        <button class="btn btn-primary btn-sm" onclick={openProposal}>
          <FileText size={16} class="size-4" /> Create Proposal
        </button>
      {/if}
...
{#if proposalOpen}
  <Modal size="lg" label="Create proposal">
    <header class="flex items-center justify-between px-6 pt-5 pb-3 border-b border-base-300">
      ...
      <button type="button" class="btn btn-ghost btn-sm btn-square" onclick={() => (proposalOpen = false)} aria-label="Close"><X size={20} class="size-5" /></button>
    </header>
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          ...
          <button type="button" class="btn btn-ghost btn-xs" onclick={() => (proposalLines = [...proposalLines, { description: '', quantity: 1, unitPrice: 0 }])}>
            <Plus size={12} class="size-3" /> Add line
          </button>
        </div>
        <div class="space-y-2">
