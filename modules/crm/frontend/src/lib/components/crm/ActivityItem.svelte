<script lang="ts">
  import { CheckCircle, Phone, Mail, CalendarDays, MonitorPlay, FileText, CheckSquare } from 'lucide-svelte';
  ...
  const ActivityIcon = $derived(ICONS[activity.type as keyof typeof ICONS] ?? FileText);
...
<div class="flex items-start gap-3 py-2.5 border-b border-base-300/50 last:border-0">
  <div class="mt-0.5 p-1.5 rounded-lg bg-base-300/50 shrink-0">
    <ActivityIcon size={14} class="size-3.5 opacity-60" />
  </div>
  ...
  {:else if done}
    <CheckCircle size={16} class="size-4 shrink-0 text-success opacity-60 mt-0.5" />
  {/if}
</div>
