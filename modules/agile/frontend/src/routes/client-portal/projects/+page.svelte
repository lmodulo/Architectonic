<script lang="ts">
  import { Download, FolderKanban, Paperclip } from 'lucide-svelte';
...
<div class="p-6 max-w-5xl mx-auto">
  <div class="flex items-center gap-3 mb-6">
    <FolderKanban size={24} class="w-6 h-6 opacity-60" />
    <div>
  ...
  {#if milestones.length === 0}
    <div class="text-center py-16 opacity-40">
      <FolderKanban size={40} class="w-10 h-10 mx-auto mb-3" />
      <p>No projects yet</p>
    </div>
  {:else}
            ...
            {#if m.attachments?.length > 0}
              <div class="mt-3 border-t border-base-200 pt-3">
                <p class="text-xs font-medium opacity-60 mb-2 flex items-center gap-1">
                  <Paperclip size={12} class="w-3 h-3" /> Deliverables
                </p>
                <div class="flex flex-wrap gap-2">
                  {#each m.attachments as att}
                    ...
                    >
                      <Download size={12} class="w-3 h-3" />
                      {att.name}
                    </a>
                  {/each}
                </div>
