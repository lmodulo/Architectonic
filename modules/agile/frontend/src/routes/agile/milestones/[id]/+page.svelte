<script lang="ts">
  import { Building2, Clock, Layers, Pencil, Plus, Trash2, X } from 'lucide-svelte';
...
<div class="space-y-6 -mt-6">
      ...
      <div class="flex items-center gap-2 flex-wrap">
        ...
        {#if milestone.clientId}
          <a href="/crm/companies/{milestone.clientId}" class="badge badge-outline text-xs gap-1 hover:badge-primary transition-colors">
            <Building2 size={12} class="size-3" />{milestone.clientName}
          </a>
        {/if}
        {#if hasPermission(data.user, 'agile_milestones', 'update')}
          {#if clientEditing}
            <div class="relative flex items-center gap-1">
              <input
                ...
                autocomplete="off"
              />
              <button type="button" class="btn btn-ghost btn-xs" onclick={() => { clientEditing = false; clientQuery = ''; clientResults = []; }}><X size={12} class="size-3" /></button>
              {#if milestone.clientId}
          ...
          {:else}
            <button type="button" class="btn btn-ghost btn-xs gap-1" onclick={() => (clientEditing = true)}>
              <Pencil size={12} class="size-3" />{milestone.clientId ? 'Change client' : 'Link client'}
            </button>
          {/if}
        {/if}
      </div>
  ...
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="card bg-base-200 border border-base-300 rounded-box p-4 flex items-start gap-3">
      <div class="p-2 rounded-lg bg-success/15"><Clock size={16} class="size-4 text-success"/></div>
      <div>
    ...
    <div class="card bg-base-200 border border-base-300 rounded-box p-4 flex items-start gap-3">
      <div class="p-2 rounded-lg bg-primary/15"><Layers size={16} class="size-4 text-primary"/></div>
      <div>
    ...
    <div class="flex items-center justify-between">
      ...
      {#if hasPermission(data.user, 'agile_sprints', 'create')}
        <button class="btn btn-primary btn-sm" onclick={openSprintModal}>
          <Plus size={16} class="size-4" /> New Sprint
        </button>
      {/if}
    </div>
...
{#if sprintModal}
  <Modal size="lg" label="New Sprint">
      <header class="flex items-center justify-between px-6 pt-5 pb-3 border-b border-base-300 shrink-0">
        ...
        <button type="button" class="btn btn-ghost btn-sm btn-square" onclick={() => (sprintModal = false)}><X size={20} class="size-5"/></button>
      </header>
...
{#if editing}
  <Modal size="lg" label="Edit Milestone">
      <header class="flex items-center justify-between px-6 pt-5 pb-3 border-b border-base-300 shrink-0">
        ...
        <button type="button" class="btn btn-ghost btn-sm btn-square" onclick={() => (editing = false)}><X size={20} class="size-5"/></button>
      </header>
      <div class="p-6 space-y-4 overflow-y-auto flex-1">
        {#if editError}
