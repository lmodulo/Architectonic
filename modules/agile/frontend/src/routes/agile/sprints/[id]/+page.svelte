<script lang="ts">
  import { LayoutGrid, Plus, Trash2, X, Zap } from 'lucide-svelte';
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="card bg-base-200 border border-base-300 rounded-box p-4 space-y-2">
      <div class="flex items-center gap-2">
        <Zap size={16} class="size-4 text-warning" />
    ...
    <div class="card bg-base-200 border border-base-300 rounded-box p-4 flex items-start gap-3">
      <div class="p-2 rounded-lg bg-primary/15"><LayoutGrid size={16} class="size-4 text-primary"/></div>
      <div>
      ...
      <div class="flex items-center justify-between">
        ...
        {#if hasPermission(data.user, 'agile_jobs', 'create')}
          <button class="btn btn-primary btn-sm" onclick={openJobModal}>
            <Plus size={14} class="size-3.5" /> New Job
          </button>
        {/if}
      </div>
...
{#if sprintEditing}
  <Modal size="lg" label="Edit Sprint">
      <header class="flex items-center justify-between px-6 pt-5 pb-3 border-b border-base-300 shrink-0">
        ...
        <button type="button" class="btn btn-ghost btn-sm btn-square" onclick={() => (sprintEditing = false)}><X size={20} class="size-5"/></button>
      </header>
      <div class="p-6 space-y-4 overflow-y-auto flex-1">
        {#if sprintEditError}
