<script lang="ts">
  import { AlertCircle, Copy, Pencil, Trash2, X } from 'lucide-svelte';
...
<div class="space-y-6 -mt-6">
  ...
  <div class="space-y-4">
    ...
    <div class="space-y-1 min-w-0">
      ...
      <button
        ...
        title="Copy task ID"
      >{task.id} <Copy size={12} class="size-2.5 shrink-0" /></button>
    </div>
  ...
  {#if task.status === 'Blocked' && task.blockedReason}
    <aside class="alert alert-error p-3 rounded text-sm flex items-start gap-2">
      <AlertCircle size={16} class="size-4 shrink-0 mt-0.5" />
