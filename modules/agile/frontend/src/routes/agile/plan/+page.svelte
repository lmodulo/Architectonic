<script lang="ts">
  import { ArrowLeft, ArrowRight, ChevronDown, ChevronUp, ChevronsUpDown, ExternalLink, GitBranch, Plus, X } from 'lucide-svelte';
...
<div class="space-y-5">
  ...
  <div class="flex items-center gap-3 flex-wrap">
    ...
    {#if sprintId && hasPermission(data.user, 'agile_jobs', 'create')}
      <button class="btn btn-primary btn-sm ml-auto" onclick={openJobModal}>
        <Plus size={16} class="size-4" /> New Job in Sprint
      </button>
    {/if}
  </div>
  ...
  {:else if sprints.length === 0}
    <div class="card bg-base-200 border border-base-300 rounded-box p-12 text-center opacity-40">
      <GitBranch size={24} class="size-8 opacity-40 mx-auto mb-2" />
                ...
                <div class="flex items-start justify-between gap-2">
                  ...
                  <div class="flex items-center gap-1 shrink-0">
                    <a href="/agile/jobs/JOB-{job.jobNumber}" class="btn btn-ghost btn-xs btn-square opacity-50 hover:opacity-100" title="Open job">
                      <ExternalLink size={12} class="size-3" />
                    </a>
                    {#if hasPermission(data.user, 'agile_jobs', 'update')}
                      ...
                      >
                        {moving === job.id ? '…' : ''}
                        <ArrowRight size={12} class="size-3" />
                      </button>
                    {/if}
                  </div>
                </div>
      ...
      <section class="space-y-3">
        ...
        {#if sprintJobs.length === 0}
          <div class="card bg-base-200 border border-base-300 rounded-box p-8 text-center opacity-40 border-dashed">
            <ArrowLeft size={20} class="size-5 mx-auto mb-1" />
                    ...
                    <th class="text-xs font-semibold opacity-50 {cls}">
                      <button type="button" class="flex items-center gap-1 hover:opacity-80 transition-opacity {cls.includes('text-right') ? 'ml-auto' : ''}" onclick={() => togglePlanSort(field)}>
                        ...
                        {#if planSortField === field}
                          {#if planSortDir === 'asc'}<ChevronUp size={12} class="size-3 opacity-70" />{:else}<ChevronDown size={12} class="size-3 opacity-70" />{/if}
                        {:else}
                  ...
                  <tr class="odd:bg-transparent even:bg-black/[.025] dark:even:bg-white/[.035] hover:bg-black/[.05] dark:hover:bg-white/[.06] transition-colors">
                    ...
                    <td class="text-center">
                      {#if hasPermission(data.user, 'agile_jobs', 'update') && otherSprints.length > 0 && job.status === 'Backlog'}
                        ...
                        >
                          <ArrowLeft size={12} class="size-3" />
                        </button>
                      {/if}
                    </td>
                  </tr>
...
{#if jobModal}
  <Modal size="lg" label="New Job">
      <header class="flex items-center justify-between px-6 pt-5 pb-3 border-b border-base-300 shrink-0">
        ...
        <button type="button" class="btn btn-ghost btn-sm btn-square" onclick={() => (jobModal = false)}><X size={20} class="size-5"/></button>
      </header>
