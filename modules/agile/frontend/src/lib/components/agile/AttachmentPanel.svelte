<script lang="ts">
  import { FileText, Trash2, UploadCloud } from 'lucide-svelte';
...
<div class="space-y-2">
    ...
    <ul class="space-y-1.5">
      {#each attachments as att (att.name)}
        <li class="flex items-center gap-2 text-sm p-2 rounded bg-base-300/40 hover:bg-base-300/60 transition-colors">
          <FileText size={16} class="size-4 shrink-0 opacity-50" />
  ...
  <button
    ...
    disabled={uploading}
  >
    <UploadCloud size={24} class="size-6 mx-auto mb-1.5 {dragOver ? 'text-primary' : 'opacity-40'}" />
    {#if uploading}
