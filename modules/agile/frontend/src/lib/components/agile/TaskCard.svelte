<script lang="ts">
  import { AlertCircle, Clock, Copy, GripVertical } from 'lucide-svelte';
...
>
  ...
  <div class="flex items-start gap-1.5">
    {#if draggable}
      <GripVertical size={14} class="size-3.5 shrink-0 opacity-30 mt-0.5" />
    {/if}
  ...
  <div class="flex flex-wrap gap-1.5 items-center">
    ...
    {#if isBlocked}
      <span class="flex items-center gap-0.5 text-[10px] text-error">
        <AlertCircle size={10} class="size-2.5" /> {'Blocked'}
      </span>
    {/if}
  </div>
  ...
  {#if task.jobNumber}
    ...
    <button
      ...
      title="Copy job name"
    >{jobName} <Copy size={10} class="size-2.5 shrink-0" /></button>
  {/if}
</div>
