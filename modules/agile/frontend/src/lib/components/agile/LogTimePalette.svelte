<script lang="ts">
  import { ChevronLeft, Play, Search, X } from 'lucide-svelte';
    ...
    <div class="p-4 space-y-3">
      ...
      {#if !palTaskId}
        ...
        <div class="flex items-center gap-2 border border-base-300 rounded-lg px-3 py-2 focus-within:border-primary transition-colors">
          <Search size={16} class="size-4 opacity-40 shrink-0" />
      ...
      {:else}
        ...
        <div class="flex items-center gap-2">
          <button class="btn btn-xs btn-ghost opacity-50" onclick={() => palTaskId = ''}>
            <ChevronLeft size={14} class="size-3.5" />
          </button>
          <div class="flex-1 min-w-0">
        ...
        <div class="space-y-3 pt-1">
          ...
          <div class="border-t border-base-300 pt-2">
            ...
            ><Play size={14} class="size-3.5" />{'Start timer instead'}</button>
          </div>
        </div>
      {/if}
