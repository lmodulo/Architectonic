<script lang="ts">
  ...
  import { Search, Plus, Trash2, FolderPlus, FolderOpen, Folder, X, ChevronRight, Upload, FileText, FileImage, File } from 'lucide-svelte';
  ...
  let query        = $state('');
  let selectedFolderId = $state<string | null>(null);  // null = "All Documents"
...
<div class="space-y-6">
  <div class="flex items-start justify-between gap-4">
    <PageHeader title="Vault" subtitle="Documents, policies, and reference materials" />
  ...
  <div class="flex gap-4 items-start">
    ...
    <!-- Folder tree sidebar -->
    <div class="w-52 shrink-0 card bg-base-100 border border-base-200 overflow-hidden">
      <button
        type="button"
        class="flex items-center gap-2 w-full px-3 py-2.5 text-sm text-left transition-colors hover:bg-base-200 {selectedFolderId === null ? 'font-semibold bg-base-200' : 'opacity-70'}"
        ...
        {#each rootFolders() as folder (folder.id)}
          ...
          <div>
            <div class="flex items-center group">
              <button
                type="button"
                class="flex items-center gap-1.5 flex-1 min-w-0 px-3 py-2 text-sm text-left transition-colors hover:bg-base-200 {isSelected ? 'font-semibold bg-base-200' : 'opacity-70 hover:opacity-100'}"
              ...
              <div transition:slide={{ duration: 150 }} class="overflow-hidden">
                {#each childFolders(folder.id) as child (child.id)}
                  <div class="flex items-center group">
                    <button
                      type="button"
                      class="flex items-center gap-1.5 flex-1 min-w-0 pl-6 pr-3 py-2 text-sm text-left transition-colors hover:bg-base-200 {selectedFolderId === child.id ? 'font-semibold bg-base-200' : 'opacity-70 hover:opacity-100'}"
        ...
        <table class="table table-sm">
          ...
          <tbody>
            {#each paged as doc (doc.id)}
              {@const Icon = mimeIcon(doc.currentVersionId ? undefined : undefined)}
              <tr
  ...
  <div transition:fade={{ duration: 200 }} class="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm" role="dialog" aria-modal="true">
    <div transition:scale={{ duration: 300, start: 0.95, easing: cubicOut }} class="card bg-base-100 w-full max-w-lg shadow-xl">
      <div class="flex items-center justify-between px-6 pt-5 pb-3 border-b border-base-200">
        ...
        <button type="button" class="btn btn-ghost btn-square btn-sm" onclick={() => (uploadOpen = false)}><X class="size-5" /></button>
      </div>
      <div class="p-6 space-y-4">
        {#if uploadError}<div role="alert" class="alert alert-error text-sm">{uploadError}</div>{/if}

        <div class="form-control gap-1">
          <span class="label-text font-medium">File <span class="text-error">*</span></span>
          <input type="file" class="file-input file-input-bordered w-full" onchange={onFileChange} />
        </div>

        <div class="form-control gap-1">
          <span class="label-text font-medium">Display Name <span class="text-error">*</span></span>
          <input type="text" class="input input-bordered" bind:value={uploadForm.name} maxlength="200" placeholder="e.g. Safety Policy 2024" />
        </div>

        <div class="grid grid-cols-2 gap-4">
          <div class="form-control gap-1">
            <span class="label-text font-medium">Folder</span>
            <select class="select select-bordered" bind:value={uploadForm.folderId}>
          ...
          <div class="form-control gap-1">
            ...
            <select class="select select-bordered" bind:value={uploadForm.visibility}>
        ...
        <div class="form-control gap-1">
          ...
          <select class="select select-bordered" bind:value={uploadForm.ownerId}>
        ...
        <div class="form-control gap-1">
          ...
          <input type="text" class="input input-bordered" bind:value={uploadForm.tags} placeholder="comma-separated: policy, hr, onboarding" />
        </div>

        <div class="form-control gap-1">
          <span class="label-text font-medium">Description</span>
          <textarea class="textarea textarea-bordered" bind:value={uploadForm.description} rows="2" placeholder="Optional description…"></textarea>
        </div>
      </div>
  ...
  <div transition:fade={{ duration: 200 }} class="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm" role="dialog" aria-modal="true">
    <div transition:scale={{ duration: 300, start: 0.95, easing: cubicOut }} class="card bg-base-100 w-full max-w-md shadow-xl">
      <div class="flex items-center justify-between px-6 pt-5 pb-3 border-b border-base-200">
        ...
        <button type="button" class="btn btn-ghost btn-square btn-sm" onclick={() => (folderOpen = false)}><X class="size-5" /></button>
      </div>
      <div class="p-6 space-y-4">
        {#if folderError}<div role="alert" class="alert alert-error text-sm">{folderError}</div>{/if}
        <div class="form-control gap-1">
          <span class="label-text font-medium">Name <span class="text-error">*</span></span>
          <input type="text" class="input input-bordered" bind:value={folderForm.name} maxlength="200" placeholder="e.g. HR Policies" />
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div class="form-control gap-1">
            <span class="label-text font-medium">Parent Folder</span>
            <select class="select select-bordered" bind:value={folderForm.parentId}>
              <option value="">None (root)</option>
              {#each folders as f}
                <option value={f.id}>{f.name}</option>
              {/each}
          ...
          <div class="form-control gap-1">
            <span class="label-text font-medium">Visibility</span>
            <select class="select select-bordered" bind:value={folderForm.visibility}>
              <option value="staff">Staff</option>
              <option value="admin_only">Admin only</option>
              <option value="customer">Customer-visible</option>
            </select>
  ...
  <div transition:fade={{ duration: 200 }} class="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm" role="dialog" aria-modal="true">
    <div transition:scale={{ duration: 300, start: 0.95, easing: cubicOut }} class="card bg-base-100 w-full max-w-sm shadow-xl">
      <div class="flex items-center justify-between px-6 pt-5 pb-3 border-b border-base-200">
        ...
        <button type="button" class="btn btn-ghost btn-square btn-sm" onclick={() => (deleteFolderTarget = null)}><X class="size-5" /></button>
      </div>
      <div class="p-6 space-y-3">
        {#if deleteFolderError}<div role="alert" class="alert alert-error text-sm">{deleteFolderError}</div>{/if}
