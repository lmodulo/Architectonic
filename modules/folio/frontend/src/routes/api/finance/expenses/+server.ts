...
export const GET: RequestHandler = async ({ cookies, url }) => {
  ...
  for (const key of ['status', 'category', 'companyId', 'milestoneId', 'dateFrom', 'dateTo', 'limit', 'skip', 'sort', 'sortDir']) {
  ...
  let res: Response;
  try {
    res = await fetch(`${API_URL}/finance/expenses${suffix}`, {
...
export const POST: RequestHandler = async ({ request, cookies }) => {
  ...
  let res: Response;
  try {
    res = await fetch(`${API_URL}/finance/expenses`, {
      method:  'POST',
