      ...
      <div class="space-y-2">
        ...
        {#each editLines as item, i (i)}
          <div class="grid grid-cols-[1fr_80px_100px_32px] gap-2 items-center">
            ...
            <button type="button" class="btn btn-ghost btn-xs btn-square opacity-40 hover:opacity-100" onclick={() => removeLine(i)} disabled={editLines.length === 1}><Trash2 size={14} class="size-3.5" /></button>
          </div>
        {/each}
        <button type="button" class="btn btn-ghost btn-sm" onclick={addLine}><Plus size={16} class="size-4" /> Add line</button>
      </div>
