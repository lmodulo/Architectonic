<script lang="ts">
  import { ChevronDown, ChevronUp, ChevronsUpDown, Plus, RefreshCw } from 'lucide-svelte';
...
<div class="space-y-6">
  ...
  {#if subscriptions.length === 0}
    <div class="card bg-base-200 border border-base-300 rounded-box p-8 text-center">
      <RefreshCw size={32} class="size-8 opacity-20 mx-auto mb-2" />
              ...
              <th class={cls}>
                <button type="button" class="flex items-center gap-1 hover:opacity-80 transition-opacity" onclick={() => applySort(field)}>
                  ...
                  {#if filters.sort === field}
                    {#if filters.sortDir === 'asc'}<ChevronUp size={12} class="size-3 opacity-70" />{:else}<ChevronDown size={12} class="size-3 opacity-70" />{/if}
                  {:else}
