<script lang="ts">
  import { Plus, Receipt, X, ChevronsUpDown, ChevronUp, ChevronDown } from 'lucide-svelte';
  ...
  const SortIconExpenseNumber = $derived(sortIcon('expenseNumber'));
  const SortIconVendor        = $derived(sortIcon('vendor'));
  const SortIconCategory      = $derived(sortIcon('category'));
  const SortIconExpenseDate   = $derived(sortIcon('expenseDate'));
  const SortIconAmount        = $derived(sortIcon('amount'));
...
<div class="space-y-4">
  ...
  {#if expenses.length === 0}
    <div class="card bg-base-200 border border-base-300 rounded-box p-12 text-center">
      <Receipt size={40} class="size-10 opacity-20 mx-auto mb-3" />
