...
<div class="space-y-6 -mt-6">
  <div class="space-y-4">
    ...
    <div class="space-y-1 min-w-0">
      <h1 class="text-2xl font-bold">New Estimate</h1>
    </div>
  </div>
  ...
  <form method="POST" use:enhance={() => { submitting = true; return async ({ update }) => { submitting = false; await update(); }; }}>
    ...
    <div class="card bg-base-200 border border-base-300 rounded-box p-5 space-y-4 mb-6">
      ...
      <div class="space-y-1">
        <label class="text-xs font-medium opacity-60 uppercase tracking-wide" for="title">Title (optional)</label>
        <input id="title" name="title" type="text" class="input w-full" placeholder="e.g. Website Redesign 2026" />
      </div>
      <div class="grid grid-cols-2 gap-4">
        ...
        <div class="space-y-1">
          <label class="text-xs font-medium opacity-60 uppercase tracking-wide" for="validUntil">Valid until</label>
          <input id="validUntil" name="validUntil" type="date" class="input w-full" />
        </div>
        <div class="space-y-1">
          <label class="text-xs font-medium opacity-60 uppercase tracking-wide" for="currency">Currency</label>
    ...
    <div class="flex gap-3 justify-end">
      <a href="/folio/estimates" class="btn btn-ghost">Cancel</a>
