...
import { sendInvoiceOverdueEmail } from '../../lib/invoiceEmail.js';
...
export function startBillingScheduler(app: FastifyInstance) {
  const db = app.mongo.db!;
  app.log.info('billing scheduler tick');
  tick(db).catch(err => app.log.error(err));
  setInterval(() => { tick(db).catch(err => app.log.error(err)); }, 60 * 60 * 1000).unref();
}
